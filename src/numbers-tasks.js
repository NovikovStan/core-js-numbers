...
function roundToPowerOfTen(num, pow) {
  const extra = 10 ** pow;
  const numExtra = num % extra;
  let result = 0;
  if (numExtra <= 5 * 10 ** (pow - 1)) result = num - numExtra;
  else result = num - numExtra + 10 ** pow;
  return result;
}
...
function toNumber(value, def) {
  let result;
  if (Number.isNaN(+value)) result = def;
  else result = +value;
  return result;
}
